...
layui.use(['layer','element','jquery'],function() {
    ...
    $('.left-nav #nav').on('click', 'li', function(event) {
        ...
        if($('.left-nav').css('width')=='60px'){
          ...
          $('#side-nav').css('top', '268px');
          $('.left-nav cite,.left-nav .nav_right').show();
          $('.left-nav .desc-head-m').hide();
          $('.left-nav .desc-name, .left-nav .desc-head, .left-nav .desc-position').show();
        }
    ...
    $('.page-content #tool').on('mouseenter', 'i', function(event) {
        var tips  = $(this).attr('lay-tips');
        left_tips_index = layer.tips(tips, $(this));
    });

    $('.page-content #tool').on('mouseout', 'i', function(event) {
        layer.close(left_tips_index);
    });
    ...
    $('.container .left_open i').click(function(event) {
        if($('.left-nav').css('width')=='220px'){
            $('.left-nav .open').click();
            $('.left-nav i').css('font-size','22px');
            $('.left-nav').animate({width: '60px'}, 100);
            $('.admin-desc').css('padding', '2em 0');
            $('#side-nav').css('top', '108px');
            $('.left-nav cite,.left-nav .nav_right').hide();
            $('.left-nav .desc-name, .left-nav .desc-head, .left-nav .desc-position').hide();
            $('.left-nav .desc-head-m').show();
